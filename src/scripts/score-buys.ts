...
async function main() {
  ...
  console.log("Buy candidates (top 25)");
  console.log("  entryId:", entryId, " eventId:", eventId);
  console.log("");
  for (const row of scores) {
    const webName = webNameById.get(row.playerId) ?? `#${row.playerId}`;
    const costStr = (row.features.nowCost / 10).toFixed(1);
    const ownershipStr =
      row.features.selectedByPercent !== null
        ? `${row.features.selectedByPercent}%`
        : "—";
    const reasonsStr = row.reasons.length > 0 ? row.reasons.join(", ") : "—";
    console.log(
      `  ${webName}  team=${row.features.teamId} pos=${row.features.positionId}  cost=£${costStr}  ownership=${ownershipStr}  score=${row.buyScore}  ${reasonsStr}`
    );
  }
}
